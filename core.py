...
def sessions_status(board: str) -> dict[str, Any]:
    ...
    try:
        ...
        if independent_roles:
            roots.append({"root_id":"__independent__","title":"Independent tasks","status":"running","collapsed":True,"attention":sum(1 for r in independent_roles if r.get('pending_approval')),"roles":independent_roles})
...
function renderRoleSide(){let html='<div class="board-group"><div class="board-title">Roles</div><div class="root open">'; const catalog=roleCatalog(); if(!catalog.length){html+='<div class="small">Open a board-specific cockpit to use role shortcuts.</div>'} for(const rr of catalog){const label=(isAll?shortBoard({board:rr.board,board_title:rr.board_title})+' / ':'')+rr.role; html+=`<button draggable="true" class="chip role-def ${rr.active?'active':'idle'}" title="Open ${esc(label)} independent chat" data-role="${esc(rr.role)}" data-board="${esc(rr.board)}">${rr.active?'●':'○'} ${esc(label)} <span class="small">${rr.active?'active':'idle'}</span></button>`} html+='</div></div>'; return html}
function renderSessionSide(){let html=''; let lastBoard=null; for(const root of sessions.roots){const b=shortBoard(root); if(b!==lastBoard){if(lastBoard!==null)html+='</div>'; html+=`<div class="board-group"><div class="board-title">${esc(b)}</div>`; lastBoard=b;} const key=rootKey(root);const collapsed=root.collapsed&&!expandedRoots.has(key);html+=`<div class="root ${collapsed?'collapsed':'open'}"><div class="root-title ${collapsed?'closed':'open'}" title="${esc(root.title)}" data-root="${esc(key)}"><span>${collapsed?'▸':'▾'}</span><span class="root-name">${esc(shortRoot(root))}</span><span class="root-state">${esc(rootBadge(root))}</span></div>`; if(!collapsed){for(const r of root.roles){html+=`<button draggable="true" class="chip ${cls(r)}" title="${esc(r.title||'')}" data-task="${r.task_id||''}">${sym(r.task_status,r.pending_approval)} ${esc(r.role)} <span class="small">${esc(displayStatus(r))}</span>${paneRef(r.task_id)}</button>`}} html+='</div>'} if(lastBoard!==null)html+='</div>'; return html}
