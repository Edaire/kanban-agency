...
def _wait_for_tui_ready(tmux_name: str, timeout: float = 8.0) -> str:
    ...
    while time.time() < deadline:
        try:
            ...
            last = ""
        low = last.lower()
        if (not trust_answered) and "do you trust the contents of this directory" in low:
            subprocess.run(["tmux", "send-keys", "-t", str(tmux_name), "1", "Enter"], check=False, env=_tmux_env())
            trust_answered = True
            time.sleep(0.75)
            continue
